...
export default function BrainTumorModal({ id }: { id: string }) {
  ...
  // Handle form submit to predict brain tumor
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setError("An error occurred while predicting the brain tumor.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmitToBackend = async (e: FormEvent<HTMLFormElement>) => {
    ...
    const form = {
      testId: id,
      result: result.predicted_brain_tumor,
    };

    try {
    //   const res = await axios.post(`${API_BASE_URL}/testReports/brainTumor/create`, form);
